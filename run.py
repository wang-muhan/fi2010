...
def train(config: Config, trainer: L.Trainer, run=None):
    ...
    if dataset_type == "FI_2010":
        ...
        if config.model.type in (cst.ModelType.ADALNMLPLOB, cst.ModelType.MLPT):
            ...
            train_set = Dataset(train_input, train_labels, seq_size, indices=train_indices, stock_labels=train_stock_ids)
            val_set = Dataset(val_input, val_labels, seq_size, indices=val_indices, stock_labels=val_stock_ids)
            test_set = Dataset(test_input, test_labels, seq_size, indices=test_indices, stock_labels=test_stock_ids)
        else:
    ...
    class_weights = None
    if getattr(config.experiment, "use_class_weight", False):
        num_classes = counts_train[1].shape[0]
        total = train_labels.shape[0]
        class_weights = total / (num_classes * counts_train[1].float())
    print()
    ...
    if "TRAINING" in experiment_type or "FINETUNING" in experiment_type:
        ...
        for i in range(len(test_loaders)):
            ...
            if run is not None and dataset_type == "FI_2010":
